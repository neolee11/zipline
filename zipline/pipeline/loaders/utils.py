...
def next_event_indexer(all_dates,
                       ...
                       event_sids):
    """
    Construct an indexer that will convert a column of event data into a 2D
    array containing a value for the most recent next-known event for each sid
    on each date.
...
def previous_event_indexer(all_dates,
    ...
    """
    Construct an indexer that will convert a column of event data into a 2D
    array containing a value for the most recent previously-known event for
    each sid on each date.
