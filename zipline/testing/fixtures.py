...
class WithPipelineEventDataLoader(ZiplineTestCase, WithAssetFinder):
    """
    ...
    def pipeline_event_setup_engine(self, dates):
        """
        Make a Pipeline Engine object based on the given dates.
        """
    ...
    # def build_expected_results(self,
    #                            dates,
    #                            date_intervals,
    #                            raw_column_specs):
    #     """
    ...
    #     """
    #     def calendar_subslice(bounds):
    #         return dates[dates.slice_indexer(*bounds)]

    #     date_intervals = map(calendar_subslice, event_intervals)
    #     nrows = sum(map(len, date_intervals))

    #     column_data = {
    #         name: np.full(nrows, missing_value, dtype=dtype)
    #         for name, (dtype, missing_value) in raw_column_specs.items()
    #     }
